...
New home and substantial skill content updates. The plugin moves to its own dedicated repo and the bundled `SKILL.md` picks up the recap capabilities + long-kernel guidance that landed in the canonical mumo skill since 0.2.2.

- **New repo: `mumo-chat/mumo-claude`** — previously the plugin shipped from `mumo-chat/mumo-mcp`. `mumo-mcp` is now repurposed as the canonical baseline + build system for all mumo MCP client packages (Claude Code, Codex, Cursor, VS Code, Hermes Agent, OpenClaw); each client repo's `SKILL.md` is generated from the shared `SKILL.template.md` via `scripts/build-skill.js`. The Claude-specific frontmatter, setup section, and `target` path live in `mumo-mcp/scripts/clients/claude-code/`. `plugin.json` and `marketplace.json` `repository` fields now point at `mumo-chat/mumo-claude`.
